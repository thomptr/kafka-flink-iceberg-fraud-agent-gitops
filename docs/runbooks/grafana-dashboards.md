...
**If variables stay N/A or panels show No data:** Strimzi Grafana dashboards expect `strimzi_io_*` labels on scraped series. Ensure the `PodMonitor` includes the `relabelings` from Strimzi’s metrics examples (`strimzi-kafka-podmonitor.yaml` in this repo).
